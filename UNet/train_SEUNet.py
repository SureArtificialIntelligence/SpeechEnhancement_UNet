...
parser.add_argument('--monitor_seen_wav', type=str,
                    default='/nas/staff/data_work/Sure/GPU_wav_dump/SE_UNet/UNet/wSDR/monitor_seen_wav')
parser.add_argument('--monitor_unseen_wav', type=str,
                    default='/nas/staff/data_work/Sure/GPU_wav_dump/SE_UNet/UNet/wSDR/monitor_unseen_wav')
...
if __name__ == '__main__':
    ...
    if args.optimiser == 'amsgrad':
        optimiser_G = Adam(ss.parameters(), lr=args.lr, betas=(0.5, 0.999), amsgrad=True)
    else:
        optimiser_G = Adam(ss.parameters(), lr=args.lr, betas=(0.5, 0.999))
    ...
    for epoch in range(args.Epoch):
        for idx, batch_tr in enumerate(dl_tr):
            ...
            if global_steps % args.save_and_test_every == 0 and avg_loss_tr < args.loss_threshold:
                ...
                torch.save(ss, pjoin(args.save_models, 'seunet_{}.pkl'.format(global_steps)))
                print('-------------------------------- Validation ---------------------------')
                loss_va = 0.
                counter_va = 0
