...
class DataETL(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        snr = self.snr_generator(path_name=audio_paths['signal'] + audio_paths['noise'])
        snrs = {'sn': snr}
        if self.existPreserve:
            snr_preserve = self.snr_generator(path_name=audio_paths['signal'] + audio_paths['preserve'])
    ...
    def snr_generator(self, snr_range=[0, 5, 10, 15, 20], path_name=None):
        if self.partition == 'train':
