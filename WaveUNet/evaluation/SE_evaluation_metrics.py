...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(root_wav_dir):
        for file in tqdm(files[:10]):
            if file.endswith('.wav') and file.startswith('denoised'):
                ...
                noisy_path = enhanced_path.replace('denoised', 'noisy')[:-11] + '.wav'
                clean_path = enhanced_path.replace('denoised', 'clean')[:-11] + '.wav'
    ...
    avg_csig_original, avg_cbak_original, avg_covl_original, avg_pesq_original = [np.mean(results_original_all[em]) for em in considered]
    avg_csig_enhanced, avg_cbak_enhanced, avg_covl_enhanced, avg_pesq_enhanced = [np.mean(results_enhanced_all[em]) for em in considered]
